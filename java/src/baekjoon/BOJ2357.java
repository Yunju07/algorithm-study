...
class BOJ2357 {
    static long[] maxTree = new long[400001];
    ...
    public static long getMax(int idx1, int idx2, int node, int nodeL, int nodeR) {
        // 구간 밖이면
        ...
        return Math.max(getMax(idx1, idx2, node*2, nodeL, mid),
                getMax(idx1, idx2, node*2 + 1, mid+1, nodeR));
    }
    ...
    public static long updateMax(int idx, long value, int node, int nodeL, int nodeR) {
        // 구간 밖이면
        ...
        return maxTree[node] = Math.max(updateMax(idx, value, node*2, nodeL, mid),
                updateMax(idx, value, node*2 + 1, mid+1, nodeR));
    }
